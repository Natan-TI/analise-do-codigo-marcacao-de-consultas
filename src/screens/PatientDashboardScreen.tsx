/**
 * screens/PatientDashboardScreen.tsx
 * Responsabilidade: Tela principal para pacientes.
 * Mostra suas consultas agendadas, permite navegar para agendar novas,
 * acessar o perfil, abrir configurações ou fazer logout.
 */
...
// Tipagem para styled-components que recebem o status
interface StyledProps {
  status: string;
}

// Função auxiliar que retorna cor com base no status
...
// Função auxiliar que retorna texto legível para o status
...
const PatientDashboardScreen: React.FC = () => {
  ...
  // Estado das consultas do paciente
  const [appointments, setAppointments] = useState<Appointment[]>([]);
  const [loading, setLoading] = useState(true);

  // Carrega consultas do paciente logado no AsyncStorage
  const loadAppointments = async () => {
    try {
      const storedAppointments = await AsyncStorage.getItem('@MedicalApp:appointments');
      if (storedAppointments) {
        const allAppointments: Appointment[] = JSON.parse(storedAppointments);
        // Filtra somente as do paciente logado
...
// Badge que mostra o status da consulta
