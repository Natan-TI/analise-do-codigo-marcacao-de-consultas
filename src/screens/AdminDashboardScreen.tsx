 ...
 * Responsabilidade: Tela do administrador, onde ele visualiza estatísticas do sistema, 
 * especialidades mais procuradas, últimas consultas e pode gerenciar usuários.
 * Fluxo: ao focar na tela → carrega consultas, usuários e estatísticas do AsyncStorage/serviço →
 * renderiza cards de estatísticas, especialidades e lista de últimas consultas →
 * permite atualizar status de consultas (confirmar/cancelar) →
 * botões para navegar até gerenciamento de usuários, perfil e sair da aplicação.
 * Por quê: centraliza as principais ações e informações para o administrador do sistema.
 */
...
// ===== Utilitários para exibir status das consultas =====
...
const AdminDashboardScreen: React.FC = () => {
  ...
  // Estados para consultas, usuários e estatísticas
  const [appointments, setAppointments] = useState<Appointment[]>([]);
  const [users, setUsers] = useState<User[]>([]);
  const [statistics, setStatistics] = useState<Statistics | null>(null);
  const [loading, setLoading] = useState(true);

  // ===== Função: carregar dados de consultas, usuários e estatísticas =====
  const loadData = async () => {
    try {
      // Recupera consultas salvas
  ...
  // ===== Efeito: carrega dados toda vez que a tela ganha foco =====
  useFocusEffect(
    React.useCallback(() => {
      loadData();
    }, [])
...
// ===== Estilos adicionais (fora do styled-components) =====
const styles = {
  scrollContent: {
    padding: 20,
  },
