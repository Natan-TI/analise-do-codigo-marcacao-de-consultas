 ...
 * Responsabilidade: Tela do painel do médico, onde ele acompanha suas consultas,
 * estatísticas de atendimento e pode confirmar/cancelar agendamentos.
 * Fluxo: carrega consultas do AsyncStorage → exibe estatísticas → médico interage
 * confirmando ou cancelando consultas → atualiza status e notifica o paciente.
 */
...
// Tipagem para estilização do status
interface StyledProps {
  status: string;
}

// Função utilitária: retorna cor baseada no status
...
// Função utilitária: retorna texto baseado no status
...
const DoctorDashboardScreen: React.FC = () => {
  const { user, signOut } = useAuth(); // Recupera usuário logado e função de logout
  ...
  // ===== Carrega consultas e estatísticas =====
  const loadAppointments = async () => {
    try {
      ...
      // Carrega estatísticas específicas do médico
      if (user?.id) {
  ...
  // ===== Abertura do modal de ação (confirmar/cancelar) =====
  ...
  const handleConfirmAction = async (reason?: string) => {
    ...
    try {
      ...
      if (storedAppointments) {
        ...
        // Atualiza status da consulta escolhida
        const updatedAppointments = allAppointments.map(appointment => {
          if (appointment.id === selectedAppointment.id) {
            return { 
              ...appointment, 
              status: actionType === 'confirm' ? 'confirmed' : 'cancelled',
              ...(reason && { cancelReason: reason }) // adiciona motivo se cancelado
            };
          }
          return appointment;
        });
        ...
        // Notifica paciente dependendo da ação
        ...
        loadAppointments(); // Recarrega lista atualizada
      }
    } catch (error) {
  ...
  return (
    <Container>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          appointments.map((appointment) => (
            <AppointmentCard key={appointment.id}>
              <ListItem.Content>
                ...
                {/* Ações disponíveis apenas se consulta estiver pendente */}
        ...
        {/* Modal de ação (confirmar/cancelar consulta) */}
        {selectedAppointment && (
...
// ===== Estilos adicionais (não estilizados via styled-components) =====
const styles = {
